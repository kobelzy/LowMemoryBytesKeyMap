...
public class BytesToLongMap extends HashMap<Integer, KVSortByteArray> {
    ...
    public void put(String key, long value) {
        ...
        int keyIndex = getKeyIndex(keyBytes);
        if (!this.containsKey(keyIndex)) this.put(keyIndex, new KVSortByteArray(16, keyBytes.length, 8));
        KVSortByteArray sortByteArray = this.get(keyIndex);
        sortByteArray.add(keyBytes, ByteUtils.LongToBytes(value));
    }
    ...
    public long get(String key) {
        ...
        if (this.containsKey(keyIndex)) {
            return this.get(keyIndex).getLong(keyBytes);
        }
        return -1L;
    }
    ...
    public String getStr(String key) {
        ...
        int keyIndex = getKeyIndex(keyBytes);
        if (this.containsKey(keyIndex)) {
            long searchValue = this.get(keyIndex).getLong(keyBytes);
    ...
    private int getKeyIndex(String key) {
        return getKeyIndex(key.getBytes());
    }

    private int getKeyIndex(byte[] key) {
        return key.length * 100 + CHARS.indexOf(key[0]);
    }
}
